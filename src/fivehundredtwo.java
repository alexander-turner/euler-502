...
public class fivehundredtwo {
	// width, height
	static Castle globalCastle = new Castle(4,13);
	 ...
	 */
	static List<ArrayList<Move>> cachedMovesRec = new ArrayList<ArrayList<Move>>(globalCastle.width+1);
	
	// IN PROGRESS: castleResults[x][y] stores even and odd solutions for castles of dimensions x by y (counting the base)
	static Result[][] castleResults = new Result[globalCastle.width+1][globalCastle.height+1];
	...
	static void prepCachedMovesRec(){
		cachedMovesRec = new ArrayList<ArrayList<Move>>(globalCastle.width+1);
		cachedMovesRec.add(new ArrayList<Move>());
	 ...
	 */
	static BigInteger enumerateCastleRec(int spaceIndex){
	...
	static Result memoiseCastleWrapper(int w, int h){
	 ...
	 */
 	static Result memoiseCastle(int spaceIndex){
	...
	static Result memoiseCastleCorrect(int spaceIndex){
		Result sum = new Result(); 
		if(globalCastle.isSolution()){
			if(globalCastle.blockIDEven())
				sum.incrementEven();
...
class Castle{
	...
	public int current; // the current row index
	private int placedInRow[]; // how many blocks have been placed in each row
	public int spacesInRow[]; // how many spaces are in each row
	public boolean skipSpace; // global flag for knowing when to move to next space
	private boolean lastIDEven; // track whether the ID of the last block placed is even
	private boolean unavailableColumn[];
	public List<ArrayList<Space>> spaces; // track available spaces for each level - update while placing / removing blocks
	...
	public Castle(int w, int h){
		...
		this.spaces = new ArrayList<ArrayList<Space>>(this.height);
	 ...
	 */
	public int placeBlockUpdate(Move m, int spaceIndex){
	...
	public void removeBlockUpdate(Move m, int spaceIndex){
		int leftSide = m.getIndex()-1, rightSide = m.getIndex()+m.getWidth();
	 ...
	 */
	void modifySpaceAbove(Move m, boolean addTrue){
	 ...
	 * Postconditions: 
	 *  Revalidates columns previously unavailable due to spaces / clears all rows above with zeroes 
	 */
...
class Result{
	...
	public Result(int even, int odd) {
...
class ResultTRIE{
	...
	public Result getResult(ArrayList<Integer> list){
		if(list.isEmpty())
			return this.data;
		else
